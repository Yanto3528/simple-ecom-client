...
import { ClassValue, Config, ConfigSchema, Props } from './tailwind-variant.types';
...
export const tv =
  ...
  ({ className, ...props }: Props<T>) => {
    ...
    const { variants, defaultVariants, compoundVariants, variantsFunction } = config;

    const variantClassNames = Object.keys(variants).map((variantKey) => {
      const variantPropsValue = props[variantKey as keyof typeof props] as string;
    ...
    const compoundVariantClassNames = compoundVariants
      ? compoundVariants.map(({ className: compoundClassName, ...variantType }) => {
